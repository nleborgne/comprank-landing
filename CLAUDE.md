...
- **Branch `prototype/structure-trois-recits` only** (throwaway, ticket #18): `components/proto-structure.tsx` holds low-fi sections for four candidate page structures, `app/page.tsx` wraps everything after the hero in a `/ideas` picker (`data-uidotsh-pick`, five options incl. current), and `app/layout.tsx` loads `https://ui.sh/ui-picker.js` via `next/script`. Duplicate section `id`s across hidden options are expected there; none of this is meant to reach `main`.
