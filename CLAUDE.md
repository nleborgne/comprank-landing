...
- **Branch `prototype/structure-trois-recits` only** (throwaway, ticket #18, structure A chosen 2026-08-30): `components/proto-structure.tsx` holds the low-fi `ProtoJudgeAppSection` (replaces `JudgeScoringSection`, hosts the badges) and `ProtoOfferTwoPanels` (Régie live + TV live / chrono RFID, tagged « Offre à part »), rendered in `app/page.tsx` between the planner and the 5-card grid. Primary source for the implementation ticket #23; none of it is meant to reach `main` as is.
