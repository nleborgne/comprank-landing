...
- **`lib/lead-schema.ts`** — shared zod schema (client + server), French error messages, pill option lists, and label helpers for the email body. Exports `LEAD_MAX_LENGTHS` (max bounds on all string fields, enforced in the schema and mirrored as input `maxLength`) and `isHoneypotFilled` (single definition of what counts as a bot submission, used by both the server action and the Lead pixel gate).
